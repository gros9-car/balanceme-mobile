...
// Recupera registros recientes para calcular la racha e identificar el ├║ltimo estado.
useEffect(() => {
  if (!user?.uid) {
    setCurrentStreak(0);
    setLastMood('calm');
  ...
  // Controla la animaci├│n del men├║ lateral y su visibilidad.
  const toggleMenu = () => {
    if (menuVisible) {
  ...
  // Navega seg├║n la opci├│n seleccionada y maneja acciones especiales como logout.
  ...
  // Convierte el ├║ltimo estado de ├ínimo guardado en su emoji correspondiente.
